...
**Concurrent installation:** Multiple envy instances may attempt self-deployment simultaneously (e.g., parallel CI jobs, multiple terminals). Envy uses the same cache locking strategy as spec and package installation: acquire exclusive lock on `$CACHE/locks/envy.$VERSION.lock`, check if already deployed (another process may have finished), deploy if still missing, release lock. Lock-free fast path: if `$CACHE/envy/$VERSION/envy` exists, skip locking entirely.
...
PACKAGES = {
    { spec = "local.python@v1", source = "envy/specs/python.lua" },
    {
        spec = "arm.gcc@v2",
