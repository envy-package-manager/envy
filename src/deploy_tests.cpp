...
TEST_CASE("deploy: stamp_product_script differs by project root hop") {
  std::string const up{ envy::deploy_stamp_product_script("foo",
                                                          envy::platform_id::POSIX,
                                                          "..") };
  std::string const here{ envy::deploy_stamp_product_script("foo",
                                                            envy::platform_id::POSIX,
                                                            ".") };
  CHECK(up != here);
}
...
TEST_CASE("deploy: verify_bin_dir accepts the manifest's own directory as the bin dir") {
  ...
  auto const root{ manifest_fixture_root() };
  CHECK_NOTHROW(envy::deploy_verify_bin_dir(root / "repo",
                                            root / "repo" / "envy.lua",
                                            root_meta()));
}
